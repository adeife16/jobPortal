...
CREATE TABLE `employment` (
  ...
  `status` enum('employed','pending','reject') NOT NULL DEFAULT 'pending',
  `view` enum('yes','no') NOT NULL DEFAULT 'no',
  `user_view` enum('yes','no') NOT NULL DEFAULT 'no'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `job_post` (
  ...
  `salary_from` int(20) NOT NULL,
  `salary_to` int(20) NOT NULL,
...
ALTER TABLE `employee`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `rider_id` (`employee_id`),
  ADD UNIQUE KEY `email` (`email`),
