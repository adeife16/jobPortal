<?php
  ...
  if(isset($_GET['getUsers']) && $_GET['getUsers'] == "all")
  {
    $stmt = "SELECT * FROM employee ORDER BY id DESC";
    $users = mysqli_prepare($con, $stmt);
    mysqli_execute($users);
...
if(isset($_POST['disable']) && $_POST['disable'] != "")
{
  ...
  $stmt = "UPDATE employee SET status = 'disabled' WHERE employee_id = ?";
...
if(isset($_POST['enable']) && $_POST['enable'] != "")
{
  ...
  $stmt = "UPDATE employee SET status = 'active' WHERE employee_id = ?";
