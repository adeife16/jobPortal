$(function () {
  ...
  function getSession(){
    ...
    function setSession(session_details){
      ...
      if(sessionId == "" || sessionId === null){
        ...
        if(sessionType == 'company'){
          ...
          $("#session-link").html("<a href='job_post.php' class='color-dark' style='color: #222;'>Job Posts</a>");
        }
    ...
    function showNotice(data){
      ...
      console.log(data)
      let status;
      let session = sessionStorage.getItem("session_type");
      $("#notice").html("");
      for(let i=0; i<data.length; i++){
        if(session === "company"){
          $("#notice").append(`
            <div class="notice-box p-3" id="notice-box">
                <p style="font-weight: bolder !important;">
                  <a href="view_profile.php?userId=`+data[i].user_id+`?userType=employee" class="color-blue" target="_blank">`+data[i].first_name+` `+data[i].last_name+`</a> applied for the role of `+data[i].title+`
                </p>
                <p class="notice-chevron blue">
                  <a href="job_application.php?jobId=`+data[i].job_id+`" target="_blank" class=" color-white ml-10" style="line-height: 30px; font-size: 20px; margin: 0; padding: 5px;"><i class="ion ion-arrow-right-b"></i></a>
                </p>
            </div>
          ...
          $("#notice").append(`
            <div class="notice-box p-3" id="notice-box">
                <p style="font-weight: bolder !important;">
                  Your job application for the position of `+data[i].title+` was <strong>`+status+`</strong>
                </p>
            </div>        
          `);
        }
    ...
    function getNotice(){
      $.ajax({
        type: "GET",
        url: 'backend/notice.php?notice=new',
        cache: false,
        beforeSend: function(){
      ...
      sessionStorage.getItem("session_type")
    }
