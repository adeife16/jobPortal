...
function validate_mail($email)
{
  ...
  global $con;
  $check_table = mysqli_query($con, "SELECT c.*, r.* FROM company c, employee r WHERE c.company_email ='$email' OR r.email='$email'");
...
// check for duplicate phone number in employee and company tables
function validate_phone($phone)
{
  global $con;
  $check_table = mysqli_query($con, "SELECT c.*, r.* FROM company c, employee r WHERE c.company_number= $phone OR r.phone=$phone OR c.company_number_optional=$phone OR r.phone_option=$phone");
  if(mysqli_num_rows($check_table) > 0)
  {
    return false;
  }
